...
async function run() {
  ...
  for (const epoch of epochs) {
    ...
    if (prompt) {
      console.log("Downloading " + epoch.title);
      const encodedPrompt = encodeURIComponent(prompt);
      // use nologo=true
      const url = "https://image.pollinations.ai/prompt/" + encodedPrompt + "?width=800&height=400&nologo=true&seed=" + (epoch.order * 222);
      ...
      // Wait 5 seconds between each epoch to be absolutely safe from rate limits
      await delay(5000);
    }
  }
}
